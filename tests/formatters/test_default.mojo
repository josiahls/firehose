...
fn test_custom_format_string() raises:
    ...
    debug_assert(
        ...
        "Custom format string not set correctly, got: " + formatter.format_string.raw_format
    )
...
fn test_format_passthrough() raises:
    ...
    debug_assert(formatted.original_message == original.original_message, "Original message changed to: " + formatted.original_message)
    debug_assert(formatted.message != original.message, "Message changed to: " + formatted.message)
    debug_assert(formatted.logger_level == original.logger_level, "Logger level changed to: " + String(formatted.logger_level))
    debug_assert(formatted.message_level == original.message_level, "Message level changed to: " + String(formatted.message_level))
    debug_assert(formatted.logger_name == original.logger_name, "Logger name changed to: " + formatted.logger_name)
...
fn test_logger_integration() raises:
    ...
    debug_assert(len(messages) == 1, "Expected 1 message, got " + String(len(messages)))
    debug_assert(messages[0] != "This is a test message", "Message changed to: " + messages[0])
    ...
    for i in range(len(levels)):
        ...
        debug_assert(len(messages) == 1, "Expected 1 message, got " + String(len(messages)))
        debug_assert(messages[0] != message, "Message changed to: " + messages[0])
        outputter[][TestLoggerOutputer].clear_messages()
...
fn test_multiple_formatters() raises:
    ...
    var messages = outputter[][TestLoggerOutputer].get_messages()
    debug_assert(len(messages) == 1, "Expected 1 message, got " + String(len(messages)))
    debug_assert(messages[0] != "Test message", "Message changed to: " + messages[0])
